...
export default function Sidebar() {
  ...
  return (
    <>
        ...
        <aside className={cn(
        "bg-card text-card-foreground w-64 p-4  space-y-4 fixed inset-y-0 left-0 z-50 transform transition-transform lg:translate-x-0 lg:relative lg:shadow-none shadow-lg bg-white dark:bg-muted rounded-md",
        ...
        > */}
            ...
            >
                <Link href="/dashboard" className="flex flex-col items-center hover:text-primary hover:border-gray-600 rounded-md">
            ...
            <nav className="space-y-3 p-4">
                {navItems.map((item) => (
                    <Link
                        key={item.name}
                        href={item.href}
                        className="flex items-center space-x-2 px-3 py-2 rounded-md hover:bg-slate-500 hover:text-white transition"
                    >
